...
export const MARBLE_RUN_LEVELS: MarbleLevel[] = [
  ...
  },
  {
    id: 4,
    name: "Blocked Shaft",
    objective: "Steer around the fixed wall. Blockers are walls too, so keep the path open.",
    start: { row: 0, col: 2 },
    goal: { row: 7, col: 3 },
  ...
  },
  {
    id: 15,
    name: "Final Course",
    objective: "Combine a ramp, pipes, and a bounce pad while staying above the hazards.",
    start: { row: 0, col: 1 },
    goal: { row: 7, col: 6 },
    fixed: [hazard(4, 3), hazard(5, 4), hazard(6, 5), wall(5, 1), wrong(7, 3)],
    inventory: inventory({ ramp: 1, pipe: 3, bounce: 1 }),
    knownSolution: [
      piece(2, 1, "ramp", 0),
      piece(2, 2, "pipe", 1),
      piece(2, 3, "pipe", 1),
