...
import { DEFAULT_CHASSIS, evaluateChassis, type ChassisConfig } from "./chassis-model.ts"
...
test("a low, wide, low-weight design is stable; a tall, narrow, top-heavy one is not", () => {
  ...
  assert.ok(tippy.feedback.includes("highCenterOfMass") || tippy.feedback.includes("narrowSpacing"))
})
...
test("a slippery surface hurts traction and shows up in the straight-line test", () => {
  ...
  assert.ok(report.feedback.includes("poorTraction"))
})
...
test("high speed on a narrow base spins out on the turning test", () => {
  ...
  assert.ok(report.feedback.includes("spinOut"))
})
...
test("small wheels plus power give torque to climb the ramp", () => {
  ...
  assert.equal(weak.tests.find((t) => t.test === "smallRamp")?.result, "poor")
})
...
test("every design produces all four named tests", () => {
  ...
  assert.deepEqual(
    ...
    ["straightLine", "turning", "smallRamp", "slipperySurface"],
  )
})
