...
/** The Week 4 chatbot wording in the reader's language. */
function useS() {
  return useLanguage().t.courseUi.ai.week4Chatbot
}

let idCounter = 1000
const uid = (p: string) => `${p}-${++idCounter}`

function parseSpec(raw: string | undefined, S: Week4ChatbotStrings): ChatbotSpec {
  if (raw) {
    try {
  ...
  return getTemplate("library", S)!.build()
}
...
export function ChatbotBuilderActivity({ activity, progress }: ActivityComponentProps) {
  ...
  useEffect(() => {
    if (progress.loaded) setSpec(parseSpec(progress.progress.activities[activity.id], S))
  }, [progress.loaded, progress.progress.activities, activity.id, S])
  ...
  const issues = validateChatbot(spec, S)
  ...
  const addIntent = () => persist({ ...spec, intents: [...spec.intents, { id: uid("in"), name: S.cbNewIntent, keywords: [], response: "" }] })
  ...
  const addBranch = (id: string) => updateIntent(id, { branch: { question: S.cbWhichOption, options: [{ id: uid("op"), label: S.cbOptionA, keywords: [], response: "" }] } })
  ...
  const addOption = (intentId: string) => {
    const intent = spec.intents.find((i) => i.id === intentId)
    if (!intent?.branch) return
    updateIntent(intentId, { branch: { ...intent.branch, options: [...intent.branch.options, { id: uid("op"), label: S.cbNewOption, keywords: [], response: "" }] } })
  }
  ...
  const send = (text: string) => {
    ...
    if (!message) return
    const result = respond(spec, convo, message, S)
    setTranscript((t) => [
      ...t,
  ...
  const clearTranscript = () => {
    setTranscript([])
    setConvo(initialConvoState)
    announce(S.cbTranscriptCleared)
  }
  ...
  return (
    <ActivityFrame
      ...
      onReset={() => setConfirm({ title: S.cbResetTitle, description: S.cbResetBody, onConfirm: () => { persist(getTemplate("library", S)!.build()); clearTranscript() } })}
    >
      <p ref={announceRef} className="sr-only" role="status" aria-live="polite" />

      {/* Template */}
      <div className="mt-4">
        <span className="text-xs font-bold uppercase tracking-wide text-muted-foreground">{S.cbStartTemplate}</span>
        <div className="mt-2 flex flex-wrap gap-2">
          {templates(S).map((t) => (
            <button
              ...
              onClick={() => setConfirm({ title: `Load “${t.name}”?`, description: S.cbTemplateBody, onConfirm: () => { persist(t.build()); clearTranscript() } })}
        ...
        <p className="mt-1 text-xs text-muted-foreground">{getTemplate(spec.templateId, S)?.description}</p>
      </div>

      <div className="mt-6 grid gap-6 lg:grid-cols-2">
        {/* Editor column */}
        <div>
          <h4 className="text-sm font-bold text-foreground">{S.cbBuildBot}</h4>

          <Field label={S.cbWelcomeMessage} value={spec.welcome} onChange={(v) => setField("welcome", v)} loaded={progress.loaded} idBase="cb-welcome" />

          <p className="mt-4 text-xs font-bold uppercase tracking-wide text-muted-foreground">{S.cbIntents}</p>
            ...
            {spec.intents.map((intent, i) => (
              <li key={intent.id} className="rounded-md border border-border p-3">
                <div className="flex items-center gap-2">
                  ...
                  <IconBtn label={`Delete intent ${intent.name}`} onClick={() => setConfirm({ title: `Delete “${intent.name}”?`, description: S.cbRemoveIntentBody, onConfirm: () => removeIntent(intent.id) })}><Trash2 className="h-3.5 w-3.5" aria-hidden /></IconBtn>
                </div>
                <label className="mt-2 block text-xs text-muted-foreground">
                  ...
                  <input
                    ...
                    placeholder={S.cbKeywordsPlaceholder}
                  />
                </label>
                <label className="mt-2 block text-xs text-muted-foreground">
                  {S.cbResponse}
                ...
                {intent.branch ? (
                  <div className="mt-2 rounded-md border border-avanza-purple/30 bg-avanza-purple/5 p-2">
                    <div className="flex items-center justify-between">
                      <span className="text-xs font-bold uppercase tracking-wide text-avanza-purple-dark">{S.cbFollowUpBranch}</span>
                      <IconBtn label={S.cbRemoveBranch} onClick={() => removeBranch(intent.id)}><Trash2 className="h-3.5 w-3.5" aria-hidden /></IconBtn>
                    </div>
                    <input
                      aria-label={S.cbFollowUpQuestion}
                    ...
                    <ul className="mt-2 space-y-2">
                      {intent.branch.options.map((opt) => (
                        <li key={opt.id} className="rounded border border-border/70 p-2">
                          <div className="flex items-center gap-2">
                            <input aria-label={S.cbOptionLabel} value={opt.label} onChange={(e) => updateOption(intent.id, opt.id, { label: e.target.value })} className="flex-1 rounded border border-border bg-card px-2 py-0.5 text-xs text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-avanza-green" />
                            <IconBtn label={`Delete option ${opt.label}`} onClick={() => removeOption(intent.id, opt.id)}><Trash2 className="h-3 w-3" aria-hidden /></IconBtn>
                          </div>
                          <input aria-label={S.cbOptionKeywords} value={kwText(opt.keywords)} onChange={(e) => updateOption(intent.id, opt.id, { keywords: parseKw(e.target.value) })} placeholder={S.cbKeywordsShort} className="mt-1 w-full rounded border border-border bg-card px-2 py-0.5 text-xs text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-avanza-green" />
                          <input aria-label={S.cbOptionResponse} value={opt.response} onChange={(e) => updateOption(intent.id, opt.id, { response: e.target.value })} placeholder={S.cbResponseShort} className="mt-1 w-full rounded border border-border bg-card px-2 py-0.5 text-xs text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-avanza-green" />
                        </li>
                      ))}
                    </ul>
                    <button type="button" onClick={() => addOption(intent.id)} className="mt-2 inline-flex items-center gap-1 rounded border border-border px-2 py-0.5 text-xs font-semibold text-foreground hover:border-avanza-green/60 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-avanza-green focus-visible:ring-offset-1"><Plus className="h-3 w-3" aria-hidden />{S.cbAddOption}</button>
                  </div>
                ) : (
                  <button type="button" onClick={() => addBranch(intent.id)} className="mt-2 inline-flex items-center gap-1 rounded-md border border-border px-2 py-1 text-xs font-semibold text-muted-foreground hover:border-avanza-green/60 hover:text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-avanza-green focus-visible:ring-offset-1"><Plus className="h-3 w-3" aria-hidden />{S.cbAddBranch}</button>
                )}
              </li>
            ))}
          </ol>
          <button type="button" onClick={addIntent} className="mt-2 inline-flex items-center gap-1.5 rounded-md border border-border px-3 py-1.5 text-xs font-semibold text-foreground hover:border-avanza-green/60 hover:bg-avanza-green/5 focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-avanza-green focus-visible:ring-offset-2"><Plus className="h-3.5 w-3.5" aria-hidden />{S.cbAddIntent}</button>

          <Field label={S.cbFallbackResponse} value={spec.fallback} onChange={(v) => setField("fallback", v)} loaded={progress.loaded} idBase="cb-fallback" />
          <Field label={S.cbHumanHelpOption} value={spec.humanHelp} onChange={(v) => setField("humanHelp", v)} loaded={progress.loaded} idBase="cb-human" />
          <Field label={S.cbPrivacyBoundary} value={spec.privacyNote} onChange={(v) => setField("privacyNote", v)} loaded={progress.loaded} idBase="cb-priv" />
          <Field label={S.cbEndingNote} value={spec.endingNote} onChange={(v) => setField("endingNote", v)} loaded={progress.loaded} idBase="cb-end" />
        </div>

        {/* Outline + validation + preview column */}
        <div>
          <h4 className="text-sm font-bold text-foreground">{S.cbOutline}</h4>
          <TreeOutline spec={spec} issues={issues} />

          <div className="mt-4 rounded-md border border-border p-3">
            <p className="text-xs font-bold uppercase tracking-wide text-muted-foreground">{S.cbChecks}</p>
            {errors.length === 0 && warnings.length === 0 ? (
              <p className="mt-1 text-sm text-avanza-green-dark">{S.cbAllPass}</p>
            ) : (
          ...
          <div className="mt-4 rounded-md border border-border">
            <div className="flex flex-wrap items-center justify-between gap-2 border-b border-border px-3 py-2">
              <p className="text-sm font-bold text-foreground">{S.cbLivePreview}</p>
              <div className="flex items-center gap-2">
                <label className="inline-flex items-center gap-1 text-xs text-muted-foreground">
                  <input type="checkbox" checked={showTrace} onChange={(e) => setShowTrace(e.target.checked)} className="focus-visible:ring-2 focus-visible:ring-avanza-green" /> {S.cbShowTrace}
                </label>
              </div>
            </div>
            <p className="px-3 pt-2 text-xs text-avanza-orange-dark">{spec.privacyNote}</p>
            <div className="max-h-72 overflow-y-auto px-3 py-2" role="log" aria-label={S.cbChatTranscript} aria-live="polite">
              {transcript.length === 0 ? (
                <p className="text-sm text-muted-foreground">{S.cbTestIntro}</p>
              ) : (
                  ...
                  {transcript.map((entry) => (
                    <li key={entry.id} className={entry.role === "user" ? "text-right" : "text-left"}>
                      <span className={`inline-block whitespace-pre-line rounded-md px-3 py-1.5 text-sm ${entry.role === "user" ? "bg-avanza-green/15 text-foreground" : "bg-secondary text-foreground"}`}>
                        <span className="sr-only">{entry.role === "user" ? S.cbYouSaidPrefix : S.cbBotSaidPrefix}</span>
                        {entry.text}
                      </span>
            ...
            >
              <label htmlFor="cb-input" className="sr-only">{S.cbYourMessage}</label>
              <input id="cb-input" value={input} onChange={(e) => setInput(e.target.value)} placeholder={S.cbTypeMessage} className="flex-1 rounded-md border border-border bg-card px-3 py-1.5 text-sm text-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-avanza-green" />
              <button type="submit" aria-label={S.cbSendMessage} className="inline-flex items-center rounded-md bg-avanza-green px-3 py-1.5 text-sm font-bold text-avanza-dark hover:bg-avanza-green-dark hover:text-primary-foreground focus-visible:outline-none focus-visible:ring-2 focus-visible:ring-avanza-green focus-visible:ring-offset-2"><Send className="h-4 w-4" aria-hidden /></button>
            </form>
            <div className="flex flex-wrap gap-2 border-t border-border px-3 py-2">
              <PreviewBtn onClick={() => send("do you sell pizza on the moon")}>{S.cbTestUnexpected}</PreviewBtn>
              <PreviewBtn onClick={() => send("can I talk to a person")}>{S.cbAskHuman}</PreviewBtn>
              <PreviewBtn onClick={restart}><RotateCcw className="mr-1 h-3 w-3" aria-hidden />{S.cbRestartChat}</PreviewBtn>
              <PreviewBtn onClick={clearTranscript}>{S.cbClearTranscript}</PreviewBtn>
            </div>
          </div>
          ...
          {transcript.some((t) => t.trace) && (
            <div className="mt-4">
              <p className="text-xs font-bold uppercase tracking-wide text-muted-foreground">{S.cbTranscriptLog}</p>
              <div className="mt-1 overflow-x-auto">
                <table className="w-full border-collapse text-xs">
                  <caption className="sr-only">{S.cbTranscriptCaption}</caption>
                  <thead>
                    <tr className="text-left">
                      <th className="border-b border-border px-2 py-1 font-semibold text-foreground">{S.cbYouSaid}</th>
                      <th className="border-b border-border px-2 py-1 font-semibold text-foreground">{S.cbMatched}</th>
                      <th className="border-b border-border px-2 py-1 font-semibold text-foreground">{S.cbFallbackQ}</th>
                    </tr>
                  </thead>
                  <tbody>
